    ...
    try {
      let datas = await sequelize.query(
        ...
        SELECT c.*,gr.name AS nameGroup,gr.image AS imageGroup,gr.members, u.name,u.image AS imageUser,
            IF(FIND_IN_SET(c.id, (SELECT GROUP_CONCAT(st.chat SEPARATOR ',') FROM Status_Chat AS st WHERE st.status = 'recalls')) > 0, TRUE, FALSE) AS isRecalls, dc.dateTimeSend AS timeDeleted, GROUP_CONCAT(e.type) AS emojis, COUNT(*) AS quantities
        FROM Chats AS c INNER JOIN Group_Chats AS gr ON gr.id = c.groupChat INNER JOIN Users AS u ON u.id = c.sender
        LEFT JOIN Deleted_Chats AS dc ON c.groupChat = dc.groupChat AND dc.implementer = :sender
